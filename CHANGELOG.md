...
- Проверка SHA-256 официальной GGUF-модели во время сборки.
- Автоматический выбор диска с достаточным свободным местом для локального релиза.

### Ограничения

- Первый запуск модели может занимать до двух минут.
- Для комфортной работы рекомендуется не менее 8 ГБ оперативной памяти.
- CPU-сборка совместима с большим числом ПК, но медленнее GPU-ускорения.
