...
def test_successful_check_is_also_cached_for_one_minute():
    monitor = RateLimitMonitor(refresh_seconds=60)
    monitor.record_success("openai", now=10)
    assert monitor.can_check("openai", now=20) is False
    assert monitor.can_check("openai", now=70) is True


def test_failed_key_check_is_not_reported_as_available_and_can_be_cleared():
    monitor = RateLimitMonitor(refresh_seconds=60)
    monitor.record_error("openai", RuntimeError("invalid API key"), now=10)
    assert "failed" in monitor.describe("openai", now=20)

    monitor.clear("openai")
    assert monitor.describe("openai", now=20) == "not checked"
